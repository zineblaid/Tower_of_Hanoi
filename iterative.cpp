...
// Function to print each move
void printMove(char from, char to) {
    cout << "Move(" << from << "," << to << ");" << endl;
}

// Function to move a disk from one peg stack to another
void moveDisk(vector<int>& fromStack, vector<int>& toStack, char fromPeg, char toPeg) {
    if (fromStack.empty()) {
        // If fromStack is empty, pop from toStack to fromStack
        ...
        // If toStack is empty, pop from fromStack to toStack
...
// Iterative Tower of Hanoi function
void IterativeHanoi(int n, char S, char A, char D) {
    ...
    for (int moveNumber = 1; moveNumber <= totalMoves; moveNumber++) {
        if (moveNumber % 3 == 1) {
            ...
            moveDisk(A_stack, D_stack, A, D);  // Move between Auxiliary and Destination
        }
    }
}
